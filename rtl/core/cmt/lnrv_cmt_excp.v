...
// 来自lsu模块的异常
assign      lsu_excp_taken = cmt_lsu_buserr | cmt_lsu_misalgn;

// 来自idu模块的异常
assign      idu_excp_taken = cmt_idu_excp_ilglir;

// 来自取指模块的异常
assign      ifu_excp_taken = cmt_ifu_excp_buserr | cmt_ifu_excp_misalgn;

// 来自sys模块的异常
// 如果在debug mode下，或者dcsr寄存器中的ebreakm位没有置1，则认为ebreak指令仅产生异常，
// 否则将请求进入debug mode
assign      ebreak4excp = (d_mode | (~dcsr_ebreakm)) & cmt_sys_ebreak;
assign      sys_excp_taken = cmt_sys_ecall | ebreak4excp;

assign      csr_excp_taken = cmt_csr_idxerr;
...
assign      pipe_flush_req = cmt_vld & any_excp_taken;
...
assign      mtval_wdata =   (cmt_ifu_excp_buserr | cmt_ifu_excp_misalgn) ? exu_pc : 
                            (cmt_idu_excp_ilglir | cmt_csr_idxerr) ? exu_ir : 
                            lsu_excp_taken ? cmt_lsu_baddr : 
                            32'd0;
