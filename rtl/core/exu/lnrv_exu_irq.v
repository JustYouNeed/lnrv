...
(
    ...
    // 流水线冲刷请求
    output                      irq_pipe_flush_req,
    input                       irq_pipe_flush_ack,
    output[31 : 0]              irq_pipe_flush_pc_op1,
    output[31 : 0]              irq_pipe_flush_pc_op2,
...
assign      pipe_flush_hsked = irq_pipe_flush_req & irq_pipe_flush_ack;

assign      sft_irq_vld = sft_irq & sft_irq_en;
assign      ext_irq_vld = ext_irq & ext_irq_en;
assign      tmr_irq_vld = tmr_irq & tmr_irq_en;
...
assign      irq_pipe_flush_req = irq_taken & exu_idle & ifu_pc_vld;

// 我们默认只支持非向量中断模式，因此这里直接使用mtvec寄存器的值作为跳转地址，如果是向量模式，
// 则首先需要进行访存，以获取对应中断id的中断服务程序地址
assign      irq_pipe_flush_pc_op1 = mtvec;
assign      irq_pipe_flush_pc_op2 = 32'd0;


// 我们需要等流水线冲刷请求被接收后才请求修改相应的csr寄存器
assign      cmt_mcsr_vld = pipe_flush_hsked;
...
assign      cmt_mcause[3 : 0]   =   sft_irq_vld ? 4'd3 : 
                                    tmr_irq_vld ? 4'd7 : 
                                    ext_irq_vld ? 4'd11 : 
                                    4'd0;
