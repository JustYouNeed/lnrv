...
TEST_CASE     := ${RUN_DIR}/../../riscv-tools/riscv-tests/isa/generated/rv32ui-p-addi
DUMPWAVE     := 1
...
TEST_NAME     := $(notdir $(patsubst %.dump,%,${TEST_CASE}.dump))
TEST_RUNDIR  := ${TEST_NAME}
...
wave: 
	gvim -p ${TEST_CASE}.spike.log ${TEST_CASE}.dump &
	${WAV_TOOL} ${WAV_OPTIONS} ${WAV_INC} ${WAV_RTL} ${WAV_FILE}  & 

run: compile
	rm -rf ${TEST_RUNDIR}
	mkdir ${TEST_RUNDIR}
	cd ${TEST_RUNDIR}; ${SIM_EXEC} +DUMPWAVE=${DUMPWAVE} +TEST_CASE=${TEST_CASE} +SIM_TOOL=${SIM_TOOL} 2>&1 | tee ${TEST_NAME}.log; cd ${RUN_DIR}; 
